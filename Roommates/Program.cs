        ...
        static void Main(string[] args)
        {
            ...
            while (runProgram)
            {
                ...
                switch (selection)
                {
                        ...
                        case ("Delete a room"):
                        List<Room> roomOption = roomRepo.GetAll();
                        foreach (Room r in roomOption)
                        {
                            Console.WriteLine($"{r.Id} - {r.Name} Max Occupancy({r.MaxOccupancy})");
                        }

                        Console.Write("Which room would you like to delete? ");
                        int selectedRoomIds = int.Parse(Console.ReadLine());
                        Room selectedRooms = roomOption.FirstOrDefault(r => r.Id == selectedRoomIds);

                        roomRepo.Delete(selectedRooms.Id);
                        ...
                        case ("Delete a chore"):
                        List<Chore> choreOption = choreRepo.GetAll();
                        foreach (Chore c in choreOption)
                        {
                            Console.WriteLine($"{c.Id} - {c.Name}");
                        }

                        Console.Write("Which chore would you like to delete? ");
                        int selectedChoreIds = int.Parse(Console.ReadLine());
                        Chore selectedChores = choreOption.FirstOrDefault(c => c.Id == selectedChoreIds);

                        choreRepo.Delete(selectedChores.Id);
